/**
 * Analysis utility functions for Amazon Ads data in CommonJS format
 */
...
function analyzeAdsData(data) {
  ...
  const analyzedKeywords = keywordData.map(item => {
    let recommendation = '';
    let color = 'yellow';
    let priorityScore = 0;
    let newBid;
  ...
  const matchTypeRecommendations = [];
